<template>
  <div class="w-full">
    <!-- 文章类型列表 -->
    <div
      class="w-full h-[50px] rounded-2xl bg-white flex items-center px-[20px]"
    >
      <div
        class="flex font-bold justify-between items-center py-[2px] px-[15px] rounded-[10px] mr-[30px] cursor-pointer hover:bg-(--theme-color) hover:text-white"
        :class="{
    ...
    <div class="w-full flex flex-wrap justify-between">
      <div class="w-[49%] article rounded-[10px] p-[2px] bg-white mt-[20px] cursor-pointer hover:text-(--theme-color) relative overflow-hidden" v-show-bg
        ...
        <div class="w-full h-[270px] overflow-hidden rounded-[10px]">
            <img :src="item.image_url" alt="文章封面" class="w-full h-full object-cover hover:scale-110 transition-transform duration-300">
        </div>
        ...
        <div class="w-full h-[40px] flex pl-[20px]"> 
            <div class="flex justify-between items-center mr-[30px]" v-for="(tag, index) in articleTag" :key="index">
...
const queryArticle = async (id: number) => {
  ...
  articleList.value = res.data.list.map((rr: any) => {
    return {
        ...rr,
        create_at: moment(rr.create_at).format("YYYY-MM-DD HH:mm:ss"),
    }
  });
