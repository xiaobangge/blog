    ...
    <div ref="highlightRef">
      <div class="flex flex-col justify-between items-center rounded-xl p-[20px] pb-0 bg-[#fff] text-[#333]"
        ...
        <div class="flex w-full">
          <img :src="item?.avatar" class="w-[50px] h-[50px] rounded-full mr-[10px]" alt=""/>
          ...
          <MdEditor class="w-full text-[18px] mt-[20px] leading-[30px] px-[70px] shadow-[0px_0px_10px_rgba(0, 0, 0, 0.1)] bg-size-[100%_30px]" :class="{'backgorund': type === 'comment'}" v-model="item.content"  readonly/>
          <transtion name="fade" mode="out-in">
            <div class="p-[20px]" v-if="isShowById === item.id">
                 <write-comment v-model="content" :type="type" @reset="resetEvent" :id="item.id" :pid="item.pid || item.id" @cancel="isShowById = ''"/>
            </div>
          </transtion>
          <div class="w-full border-b-[1px] border-dashed border-[#999] mt-[20px]"></div>
          <div class="p-[20px] w-full" v-if="item.children?.length">
            <bk-comment-list :data="item.children" :level="level + 1" @comment_status="emit('comment_status', $event)"/>
          </div>
      </div>
