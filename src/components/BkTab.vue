<template>
  ...
  >
    <div
      class="flex cursor-pointer items-center justify-center mr-[50px] h-full hover:text-(--theme-color)"
      ...
      @click="active = index"
    >
      <bk-svg v-if="item.icon" :iconName="item.icon" className="w-[20px] h-[20px]"></bk-svg>
      <p class="ml-[10px] text-[16px]">{{ item.name }}</p>
    </div>
